...
if (!facebookClientId || !facebookClientSecret || !googleClientId || !googleClientSecret) {
  throw new Error(
...
export class Cognito {
  ...
  public createResources(scope: Construct) {
    ...
    this.clientWriteAttributes = new cognito.ClientAttributes().withStandardAttributes({
      fullname: true,
      email: true
    })

    this.clientReadAttributes = this.clientWriteAttributes.withStandardAttributes({
      emailVerified: true
    })
    ...
    this.facebookProvider = new cognito.UserPoolIdentityProviderFacebook(scope, 'Facebook', {
    ...
    this.googleProvider = new cognito.UserPoolIdentityProviderGoogle(scope, 'Google', {
